 ...
 *
 * This is an adaptive jitter buffer implementation. Features:
...
static struct le *packet_drop(struct jbuf *jb)
{
	struct le *le;
	struct packet *f0;
...
static bool jbuf_enough_packets(struct jbuf *jb)
{
	if (!jb->packetl.head)
		return false;
